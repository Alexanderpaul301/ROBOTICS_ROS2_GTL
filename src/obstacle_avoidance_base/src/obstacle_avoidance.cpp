...
class ObstacleAvoidance : public rclcpp::Node {
    ...
    protected: // ROS Callbacks
        ...
        geometry_msgs::msg::Twist findClosestAcceptableVelocity(const geometry_msgs::msg::Twist & desired) {
            ...
            cv::Mat_<uint8_t> Va(n_v,n_w,FREE); // Vs inter Vd // ! This matrix Va stores the acceptable values of velocity for the given map and time horizon
            cv::Mat_<uint8_t> scores(n_v,n_w,(uint8_t)OCCUPIED); // Vs inter Vd  // ! It seems like the intersection is already defined



            // Now build Va (inside Vs inter Vd) by iterating over the local
            // map, and find the most appropriate speed
            // TODO: Fill Va with the acceptable velocities (given the time horizon)
            double best_score = 0;
