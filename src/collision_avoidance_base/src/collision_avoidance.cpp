...
class CollisionAvoidance : public rclcpp::Node {
    protected:
        ...
        void scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
            ...
            for (unsigned int i=0;i<n;i++) {
                ...
                float y = msg->ranges[i]*sin(theta);    //On passe de coordonnées polaires à cartésiennes
                float z = 0;
        ...
        geometry_msgs::msg::Twist findClosestAcceptableVelocity(const geometry_msgs::msg::Twist & desired) {
            ...
            // We check if the robot is moving forward only or in any other direction.
            if (desired.linear.x > 0) {
                only_forward = true;
            }
            else {
                only_forward = false;
            }
            float speed = max_velocity*desired.linear.x/0.2;
            float speednew = 0.0;
            
            unsigned int m = lastpc.size();
            for (unsigned int i=0;i<m;i++) {
                ...
                if (only_forward && y <= safety_diameter && y >= -safety_diameter && x > 0.1 && x <= safety_diameter) {
                    speed = 0;
                    RCLCPP_INFO(this->get_logger(),"Blocked");
                }
            

                
                if (only_forward && y <= safety_diameter && y >= -safety_diameter && x > safety_diameter && x <= ignore_diameter) {
                    speednew =(max_velocity* (x - safety_diameter)) / (ignore_diameter - safety_diameter);
                    if (speed > speednew) {
                        speed = speednew;
                    }

                    RCLCPP_INFO(this->get_logger(),"Moving forward with speed: %.2f",speed);
                }
                
            }
            res.linear.x = speed;

            if (res.linear.x > max_velocity) {
                res.linear.x = max_velocity;
            }
    ...
    public:
        CollisionAvoidance() : rclcpp::Node("collision_avoidance") {
            ...
            this->declare_parameter("~/ignore_diameter", 1.5);
