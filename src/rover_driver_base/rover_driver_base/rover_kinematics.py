...
class RoverKinematics:
        ...
        if skidsteer:
            for k in drive_cfg.keys():
                ...
                # each wheel in skid-steer mode
                 ...
                 motors.steering[k] = 0
                 motors.drive[k] = (twist.linear.x - twist.angular.z * drive_cfg['C'+ k[1]].y)*500

                # if (twist.linear.x > 0 ):
                #     if (twist.angular.z > -0.2 and twist.angular.z < 0.2):
                #         motors.drive[k] = twist.linear.x +
                
                #     elif (twist.angular.z > 0.2):
                #         if (k == "FL" or k == "CL" or k == "RL"):
                #             motors.drive[k] = twist.linear.x +
                #         elif (k == "FR" or k == "CR" or k == "RR"):
                #             motors.drive[k] = 0

                #     elif (twist.angular.z < 0.2):
                #         if (k == "FR" or k == "CR" or k == "RR"):
                #             motors.drive[k] = twist.linear.x +
                #         elif (k == "FL" or k == "CL" or k == "RL"):
                #             motors.drive[k] = 0
                ...
                # elif (twist.linear.x < 0.2 and twist.linear.x > -0.2):
                #     if (twist.angular.z > -0.2 and twist.angular.z < 0.2):
                #         motors.drive[k] = 0
            ...
            for k in drive_cfg.keys():
                ...
                motors.steering[k] = (twist.linear.y + twist.angular.z * drive_cfg['C'+ k[1]].x)*200
                motors.drive[k] = (twist.linear.x - twist.angular.z * drive_cfg['C'+ k[1]].y)*200
    ...
    def prepare_inversion_matrix(self,drive_cfg):
        ...
        iw = np.asmatrix([[1,0, drive_cfg['FL'].x],
                          [0,1, drive_cfg['FL'].y],
                          [1,0, drive_cfg['FR'].x],
                          [0,1, drive_cfg['FR'].y],
                          [1,0, drive_cfg['CL'].x],
